...
- [03 표본 평균 -\> 표본 평균의 기대값과 표준오차를 통해서 모집단의 평균을 추정하자.](#03-표본-평균---표본-평균의-기대값과-표준오차를-통해서-모집단의-평균을-추정하자)
  - [표본 평균으로 모평균을 추정하기](#표본-평균으로-모평균을-추정하기)
...
- 우리는 표본 평균(sample mean)으로 모집단(상자)의 평균을 추정한다. 물론 확률 오차가 발생해서 조금 다르긴 하다. 얼마나 다른지 알기 위해서 표본 합의 SE를 구해야 한다.
- 모집단(상자)가 정규 분포가 아니더라도 표본의 합과 평균은 정규분포를 따른다. CLT 때문에.
- random sampling으로 표본을 추출했다면 표본의 SD는 모집단의 SD와 같다고 볼 수 있고, 이를 이용해 표본합의 SE를 구할 수 있게 된다. $표본\ 합의\ SE = \sqrt{표본 크기} \times SD$공식을 활용한다.
- 표본합의 SE과 CLT로 표본합, 평균의 정규분포성을 이용하여 68-95-99.7 법칙을 따라 신뢰 구간을 계산할 수 있다.

### 뭐 이리 표준 오차 종류가 많음?

확률로 추정된 값이 있으면 자연스레 확률 오차가 발생하는지라 많다.  
아래 공식에서 표본 평균의 SE는 표본 합의 SE를 단순히 표본 크기로 나누어 구할 수 있다는 점을 유의하자.

$$
표본 합의\ SE = 상자의 SD \times \sqrt{표본 크기}  \\
표본 평균의\ SE = 상자의 SD / \sqrt{표본 크기}  \\
표본 개수의\ SE = 0-1 상자의\ 표본합의\ SE   \\
