...
그러나 probability histogram은 상자 모형으로부터 계산된 합의 기대값과 표준오차에 기반하여 `합`이 특정한 값을 취할 확률을 알려주는 히스토그램이다. 따라서 모든 값을 측정하지 않는다.
...
앞서 추출 횟수가 충분히 많다면 합, 평균의 확률 분포가 정규분포를 따라간다고 적은 바 있다.
